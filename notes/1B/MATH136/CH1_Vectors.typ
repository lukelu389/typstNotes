...
#info[*Orthogonality/Angle*\
  ...
  2. With $vec(v) , vec(w)$ non-zero vectors. The angle $theta, 0 <= theta<=pi$ between $vec(v)$ is such that $ vec(w) dot vec(v) & =||vec(w)||||vec(v)|| cos theta, \
                theta & = arccos((vec(w) dot vec(v)) / (||vec(w)||||vec(v)|| )) $
...
#info[*Cross Products*
  ...
  The *cross product* of $vec(u), vec(v)$ is defined to be the vector in $RR^3$ given by $ vec(u) times vec(v) = mat(u_2 v_3 - u_3 v_2; u_3 v_1 - u_1 v_3; u_1 v_2 - u_2 v_1) $
...
#info[*Projection*\
  ...
  1. The *projection* of $vec(v)$ onto $vec(w)$ is defined by $ proj_vec(w) (vec(v)) = frac(vec(v) dot vec(w), ||vec(w)||^2) vec(w) = frac(vec(v) dot vec(w), vec(w) dot vec(w)) vec(w) $
    ...
    #align(center)[#image("../../../pictures/sem1/lineara1.png")]
...
#align(center)[#image("../../../pictures/sem1/linearach1_2.png", width: 10cm)]
